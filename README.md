...
- `Rego`, located within the PowerShell/ScubaGear folder, holds the `.rego` files. Open Policy Agent uses each Rego file to audit the desired state for each product, per the SCuBA M365 secure configuration baseline documents.
...
- `sample-report` contains sample JSON and HTML report output of the ScubaGear tool. Right click on the `BaselineReports.html` file and open the sample report in a browser of your choice.
