...
- [ ] Review draft release notes generated by the `Build and Draft Release` workflow
  - Run the workflow from the release branch (or set **compareHeadRef** to that branch). Notes compare the previous published tag to the branch head, e.g. `v1.8.0...release/1.9.0`; the new release tag is created when the release is published
  - Notes are grouped into Major Changes, Bugs Fixed, Baselines, Documentation, and Dependencies
  - In **Baselines**, move BOD 25-01 policy items into **Additions**, **Removals**, or **Updates** as needed; baseline pull requests stay under **Other baseline changes** until moved
  - Cherry-picked or other changes without a linked pull request may need to be added manually
  - Edit notes as needed before publishing (for example, trim long sections)
