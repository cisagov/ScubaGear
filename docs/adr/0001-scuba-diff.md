...
there. The record is classified by its **result** like any other pair, with
`Migrated` / `MigratedFromId` / `MigratedFromProduct` fields (and a report badge)
marking that the comparison spans the migration. The `PolicyVersionUpdate` check
is skipped for these records, since the two IDs come from different policy
families and their version suffixes are not comparable. A direct base-ID match
...
disagreeing before results. Several are also manual checks that only ever
...
Keeping the table one-to-one is enforced structurally as well: the alignment
skips a target base already claimed by an earlier entry, so a future collision
