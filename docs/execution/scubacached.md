...
```textThe most important parameter that distinguishes
OutPath\
  ├── ProviderSettingsExport.json
  ├── ScubaResults_abc123.json
  └── BaselineReports.html
...
- **`$true`**: Functions exactly like `Invoke-SCuBA` - authenticates when parameter `-LogIn $true` is used, exports provider data, runs Rego analysis, and generates reports
- **`$false` (default)**: Skips authentication and data export, runs analysis only on existing provider JSON files in the specified output path
