...
function Update-DependenciesTable {
    ...
    foreach ($line in $lines) {
        ...
        elseif ($line -match "Purpose\s*=\s*'([^']+)'") {
            $currentModule.Purpose = $matches[1]
        }
        elseif ($line -match "^\s*\}") {
            # End of module block - add to list if we have all required fields
            if ($currentModule.ModuleName -and $currentModule.ModuleVersion -and $currentModule.MaximumVersion) {
                # Set default purpose if not found
                if (-not $currentModule.Purpose) {
                    $currentModule.Purpose = 'PowerShell module dependency'
                }
    ...
    foreach ($module in $ModuleList) {
        ...
        $Purpose = $module.Purpose.PadRight(65)

        $NewRow = "| $($ModuleName.Substring(0, [Math]::Min(46, $ModuleName.Length)).TrimEnd()) | $($MinVersion.Substring(0, [Math]::Min(15, $MinVersion.Length)).TrimEnd()) | $($MaxVersion.Substring(0, [Math]::Min(16, $MaxVersion.Length)).TrimEnd()) | $($Purpose.Substring(0, [Math]::Min(65, $Purpose.Length)).TrimEnd()) |"
