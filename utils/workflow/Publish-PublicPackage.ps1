...
function Test-PublishedModule {
    ...
    try {
        ...
        if ($IsPrerelease) {
            ...
            $Module = Find-Module -Name ScubaGear -RequiredVersion $RequiredVersion -AllowPrerelease -ErrorAction Stop
        } else {
            Write-Verbose "Checking for latest stable version"
            $Module = Find-Module -Name ScubaGear -ErrorAction Stop
        }
        ...
        if ($Module) {
            ...
            throw "Module not found in PSGallery"
        }
    }
    catch {
