...
test_Spf_Correct_V1 if {
    ...
    TestResult("MS.EXO.2.2v3", Output, concat(". ", [PASS, DNSLink]), true) == true
}
...
test_Spf_Incorrect_V1 if {
    ...
    RuleOutput := [Result | some Result in Output; Result.PolicyId == "MS.EXO.2.2v3"]
    ...
    TestResult("MS.EXO.2.2v3", RuleOutput, concat("", [ReportDetailString, DNSLink]), false) == true
}

test_Spf_Incorrect_V2 if {
    # Test with 2 bad domains and one good domain
    Message1 := "SPF record found, but it does not fail unapproved senders or redirect to one that does."
    Record1 := json.patch(SpfRecords, [
        {"op": "add", "path": "domain", "value": "bad1.com"},
        {"op": "add", "path": "compliant", "value": false},
        {"op": "add", "path": "message", "value": Message1}
    ])
    Message2 := "SPF record found, but it does not fail unapproved senders or redirect to one that does."
