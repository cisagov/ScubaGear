...
InModuleScope CreateReport {
    ...
    Describe -tag "Markdown" -name 'Check Secure Baseline Markdown document exists for <Product>' -ForEach @(
        @{Product = "entraid"; MarkdownFilePath = Join-Path -Path $PSScriptRoot -ChildPath "..\..\..\..\baselines\entraid.md" -Resolve }
    ...
    Describe -tag "Markdown" -name 'Fail import secure baseline (bad path)' {
        It "Fails on bad baseline path" {
            {Import-SecureBaseline -ProductNames "entraid" -BaselinePath "garbage path"} |
            Should -Throw
        }
    }
    Describe -tag "Markdown" -name 'Check error handling.' {
        ...
        It "Check not throw an exception with malformed baseline" {
            {
                Import-SecureBaseline -ProductNames "entraid" -BaselinePath (Join-Path -Path $PSScriptRoot -ChildPath "CreateReportStubs" -Resolve)
            } | Should -Not -Throw
        }
        ...
        It "Check MS.ENTRAID.1.1v1 for malformed" {
            $Output = Import-SecureBaseline -ProductNames "entraid" -BaselinePath (Join-Path -Path $PSScriptRoot -ChildPath "CreateReportStubs" -Resolve)
            $Output.entraid.Controls[0].MalformedDescription | Should -BeTrue -Because "MS.ENTRAID.1.1v1 policy description does not start on line immediately after policy id."
            $Output.entraid.Controls[0].Value | Should -Be "Error - The baseline policy text is malformed. Description should start immediately after Policy Id."
        }
        It "Check MS.ENTRAID.2.1v1 for malformed" {
            $Output = Import-SecureBaseline -ProductNames "entraid" -BaselinePath (Join-Path -Path $PSScriptRoot -ChildPath "CreateReportStubs" -Resolve)
            $Output.entraid.Controls[1].MalformedDescription | Should -BeFalse -Because "Only warning for MS.ENTRAID.2.1v1 policy description with to many lines."
        }
    }
}