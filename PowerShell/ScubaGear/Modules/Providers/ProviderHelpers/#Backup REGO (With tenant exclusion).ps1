#Backup REGO (With tenant exclusion)
...
default CloudAppExclusionsFullyExempt(_, _) := false

# Returns true when all cloud app exclusions in the CAP are in the config allowlist
CloudAppExclusionsFullyExempt(Policy, PolicyID) := true if {
    ExemptedApps := input.scuba_config.Aad[PolicyID].CapExclusions.CloudApps
...
CloudAppExclusionsFullyExempt(Policy, PolicyID) := true if {
    ...
    count({y | y := input.scuba_config.Aad[PolicyID].CapExclusions.CloudApps}) == 0
}

default GuestUserExclusionsFullyExempt(_, _) := false

# Returns true when there are no guest exclusions on the policy (parent null or GuestOrExternalUserTypes null)
...
# Returns true when all excluded guest types are in the config allowlist and ExternalTenants scope is "all"
GuestUserExclusionsFullyExempt(Policy, PolicyID) := true if {
    ...
    GuestExclusions.ExternalTenants.MembershipKind == "all"
    input.scuba_config.Aad[PolicyID].CapExclusions.GuestExternalTenants == "all"
}

# Returns true when all excluded guest types are in the config allowlist and all enumerated tenants are in the config list
GuestUserExclusionsFullyExempt(Policy, PolicyID) := true if {
    GuestExclusions := Policy.Conditions.Users.ExcludeGuestsOrExternalUsers
    GuestExclusions != null
    ExcludedTypes := {t | some t in split(GuestExclusions.GuestOrExternalUserTypes, ",")}
    AllowedTypes := ConvertToSet(input.scuba_config.Aad[PolicyID].CapExclusions.GuestUserTypes)
    count(ExcludedTypes - AllowedTypes) == 0
    GuestExclusions.ExternalTenants.MembershipKind == "enumerated"
    AllowedTenants := ConvertToSet(input.scuba_config.Aad[PolicyID].CapExclusions.GuestExternalTenants)
    count(ConvertToSet(GuestExclusions.ExternalTenants.Members) - AllowedTenants) == 0
}
