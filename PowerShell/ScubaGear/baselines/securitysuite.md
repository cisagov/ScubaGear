...
- _Note:_ This policy is a manual check. It was previously an automated check, but automating it would require elevating ScubaGear's service principal beyond read-only. Retrieving audit log retention policies returns a 403 for a service principal holding only the Entra Global Reader role that the Security Suite checks are designed to run under; reading them requires the Compliance Administrator and Compliance Data Administrator roles, both of which grant the Purview Organization Configuration role. That role carries destructive write permissions, including the ability to delete audit log retention policies, so it is deliberately not made a prerequisite for running ScubaGear. The version suffix is intentionally unchanged at `v1`: the Security Suite baseline has not yet been officially published by CISA (it is introduced in tool version 2.0.0), so the usual rule that a change in policy meaning requires a version increment does not apply to this instance. This is deliberate, not an oversight.
