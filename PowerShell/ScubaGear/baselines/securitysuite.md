...
- _Note:_ This policy is a manual check. It was previously an automated check, but the requirement can also be satisfied by offloading audit logs out of the M365 cloud environment, which is not observable through any available API. The version suffix is intentionally unchanged at `v1`: the Security Suite baseline has not yet been officially published by CISA (it is introduced in tool version 2.0.0), so the usual rule that a change in policy meaning requires a version increment does not apply to this instance. This is deliberate, not an oversight.
