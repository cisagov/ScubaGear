...
By default, any user in the Microsft Entra ID (Entra ID) Tenant can create additional environments. Enabling these controls will restrict the creation of new environments to users with the following admin roles: Global admins, Dynamics 365 admins, and Power Platform admins.
...
When the Entra ID tenant is created, by default, a Power Platform
...
restrict users in the Entra ID tenant from creating Power Apps in the
...
Power Platform tenant isolation is different from Entra ID-wide tenant
restriction. It does not impact Entra ID-based access outside of Power
Platform. Power Platform tenant isolation only works for connectors
using Entra ID-based authentication, such as Office 365 Outlook or
...
even if the user presents valid credentials to the Entra ID-secured data
source.
...
- _Rationale:_ Provides an additional tenant isolation control on top of Entra ID tenant isolation specifically for Power Platform applications to prevent accidental or malicious cross tenant information sharing.
- _Last modified:_ June 2023
